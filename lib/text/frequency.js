...
exports.cmdscale = function (db, target, source, callback) {
  ...
  target_collection.find(target_condition, {fields: {'_id': 1}, sort: target_sort, limit: 200}).toArray(function (err, results) {
    if (err) {
      callback(err);
    } else {
      ...
      /*
       var source_sort = [];
       source_sort.push(['_id.A', 1]); // ID昇順
       source_sort.push(['_id.B', 1]); // ID昇順
       , {sort: source_sort}
       */
      ...
      source_collection.find(source_condition).toArray(function (err, results) {
        if (err) {
          ...
          var ifile = '/tmp/writetest.txt';
          var ofile = '/tmp/writetest.out';
          fs.writeFile(ifile, data.join('\n'), function (err) {
            R.exec(script, [ifile, ofile], function (err) {
              callback(err);
            });
          })
        }
