...
exports.batch = function (db, src, dst, condition, field, callback) {
  ...
  cursor.count(function (err, count) {
    ...
    cursor.each(function (err, item) {
      if (item) {
        exports.patch(db, src, item._id, collection, field, function (err) {
          if (--c == 0) {
            return callback(err, count);
          }
        });
 ...
 * @param callback
 */
exports.patch = function (db, src, _id, out, field, callback) {
  var option = {root: src};
  //_id = _id.toString();
  _id = new ObjectID(_id.toString());
//  var collectio = db.collection('mails.files');
//  GridStore.exist(db, _id, 'mails', option, function(err,exist) {
