  ...
  collection.ensureIndex(index, function (err) {
    ...
    cursor.count(function (err, count) {
      ...
      cursor.each(function (err, item) {
        if (item) {
          exports.patch(db, source.collection, item, collection, source.attribute, field, function (err) {
            if (--c == 0) {
              return callback(err, count);
            }
          });
 ...
 * @param callback
 */
exports.patch = function (db, src, item, out, attribute, field, callback) {
  var result = [];
  var util = new Util(field);
  async.eachSeries(Object.keys(item), function (key, next) {
    exports.tf(item[key], field, function (err, tf) {
      result = util.sum(result, tf);
      next();
    })
  }, function (err) {
    var data = {};
